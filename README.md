...
Notebooks are intended to be run **top-to-bottom in order**.

Raw source data are not committed to the repository. All processed panel datasets and figures are generated by the pipeline.

The project emphasizes **clarity, transparency, and reproducibility** over production deployment or performance optimization.

---

## Scope and Limitations

This analysis is designed for **policy analytics**, not causal inference.

- State–year aggregation masks within-state heterogeneity  
- SAIPE poverty estimates are model-based rather than direct survey counts  
- Unemployment is an incomplete proxy for overall economic well-being  
- Regression results should be interpreted as **descriptive associations**, not causal effects  

These limitations are intentional and reflect how economic data are commonly used in applied policy settings.
